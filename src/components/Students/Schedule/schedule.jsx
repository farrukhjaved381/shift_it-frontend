...
export default function ScheduleModule({
  ...
  return (
    <div className={`flex flex-col gap-4 sm:gap-5 ${className || ''}`}>
        {/* Top Header */}
        <header className="px-4 sm:px-6 py-4 sm:py-6 border-b border-gray-200 bg-white">
      ...
      <div className="border border-gray-200 rounded-lg bg-white shadow-sm">
        <div className="p-4 sm:p-5 border-b border-gray-200">
          ...
          <p className="text-xs sm:text-sm text-gray-500">Your clinical rotations for the current week</p>
        </div>
        <div className="p-4 sm:p-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-7 gap-3 sm:gap-4">
            {weekSchedule.map((day, index) => (
              <div key={index} className="flex flex-col gap-2 rounded-md bg-slate-100 p-2 sm:p-3">
                <div className="text-center">
                  <p className="text-xs sm:text-sm font-medium text-gray-500">
                    {day.date.toLocaleDateString("en-US", { weekday: "short" })}
                  </p>
                  <p className={`text-base sm:text-lg font-semibold ${day.date.toDateString() === new Date().toDateString() ? 'text-blue-600' : 'text-gray-900'}`}>
                    {day.date.getDate()}
                  </p>
                </div>

                <div className="flex flex-col gap-2 min-h-20 sm:min-h-30">
                  {day.rotations.map((rotation, rotIndex) => (
                    <div
                      key={`${rotation.id}-${rotation.isSession ? rotation.sessionTime : "block"}-${rotIndex}`}
                      className="p-2 rounded-md bg-blue-500 text-white text-xs"
                    >
      ...
      {/* Monthly Calendar */}
      <div className="border border-gray-200 rounded-lg bg-white shadow-sm">
        <div className="p-4 sm:p-5 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <h2 className="text-lg sm:text-xl font-semibold flex items-center gap-2">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="p-2 sm:p-3 border border-gray-300 rounded-md bg-white cursor-pointer hover:bg-gray-50 transition-colors"
            >
              ←
            </button>
            <button
        ...
        <div className="p-4 sm:p-5">
          ...
          <div className="grid grid-cols-7 gap-1">
            {calendarDays.map((day, index) => (
              <div
                key={index}
                className={`min-h-20 sm:min-h-25 p-1 border border-gray-200 rounded-md ${day?.isToday ? 'bg-blue-50' : 'bg-white'}`}
              >
                {day && (
                  <>
                    <div className={`text-xs sm:text-sm font-medium mb-1 ${day.isToday ? 'text-blue-600' : 'text-gray-900'}`}>
                      {day.day}
                    </div>
                    <div className="flex flex-col gap-1">
                      {day.rotations.map((rotation, rotIndex) => (
                        <div
                          ...
                          className="text-xs p-1 sm:p-1.5 rounded bg-gradient-to-r from-blue-500 to-blue-700 text-white shadow-sm"
